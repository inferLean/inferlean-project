...
func printUsage(w io.Writer) {
	...
	fmt.Fprintf(w, "  --duration-seconds <int>  Collection duration in seconds (default: %d)\n", defaultCollectionDurationSeconds)
	fmt.Fprintln(w, "  --duration-minutes <int>  Deprecated legacy duration override in minutes")
	...
	fmt.Fprintln(w, "  --output <path>                Write the analysis v2 JSON to this path (default: analysis-report.json)")
	...
	fmt.Fprintln(w, "  --format <value>               human or json (default: human)")
	fmt.Fprintln(w, "  --plain-output                 Disable styled terminal output and print only the report path")
	...
	fmt.Fprintln(w, "  --objective <value>       throughput, latency, or balanced (default: workload profile or balanced)")
	fmt.Fprintln(w, "  --target-p95-latency      Optional p95 latency guardrail in ms")
	fmt.Fprintln(w, "  --min-throughput          Optional minimum throughput guardrail")
	...
	fmt.Fprintln(w, "  collect + trigger backend optimization + wait for the decision-oriented report + print the same decision summary + open the Optimize workspace")
	fmt.Fprintf(w, "  default collection window is %d seconds sampled every %d seconds\n", defaultCollectionDurationSeconds, defaultPrometheusStepSeconds)
